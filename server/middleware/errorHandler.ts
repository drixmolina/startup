...
export const errorHandler: ErrorRequestHandler = (error, _request, response, _next) => {
  ...
  if (error instanceof Error && ["EMAIL_PROVIDER_FAILED", "AI_PROVIDER_FAILED"].includes(error.message)) return response.status(502).json({ success: false, error: "An upstream service is unavailable. Please try again later." })
